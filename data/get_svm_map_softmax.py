...
if __name__ == '__main__':
    ...
    T_path = "./datasets/test/RoadScene/ir/"
    RGB_path = "./datasets/test/RoadScene/vi/"

    T_file_list = sorted(os.listdir(T_path))
    RGB_file_list = sorted(os.listdir(RGB_path))

    T_map_path = "./datasets/test/RoadScene/ir_mask/"
    RGB_map_path = "./datasets/test/RoadScene/vis_mask/"
    Fusion_map_path = "./datasets/test/RoadScene/fusion_map/"
    ...
    for idx, (T_filename, RGB_filename) in enumerate(zip(T_file_list, RGB_file_list)):
        ...
        w_T, w_RGB = softmax(map_T, map_RGB, c=1)
        #img_w_T = (w_T * 255).astype(np.uint8)
        #img_w_RGB = (w_RGB * 255).astype(np.uint8)
        img_w_T = (map_T * 255).astype(np.uint8)
        img_w_RGB = (map_RGB * 255).astype(np.uint8)
